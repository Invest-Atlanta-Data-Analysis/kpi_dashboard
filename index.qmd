...
tot_inv <- data.frame(name = c("2026 YTD", "2026 Goal"), inv = c(534930837.63, 900000000))
...
#events_plot
```
[![Click the image to explore our interactive online investment map](wins_map_20260909.png){width=51%}](https://iatl.maps.arcgis.com/apps/instant/basic/index.html?appid=112649da7b1d4fa58098c16cc402dcc4){target="_blank"}
...
sb_inv <- data.frame(name = c("2026 YTD", "2026 Goal"), inv = c(2844451.98, 8500000))

sb_support <- data.frame(name = c("2026 YTD", "2026 Goal"), support = c(796, 1000))
...
nb_res <- data.frame(name = c("2026 YTD", "2026 Goal"), ppl = c(12802, 10000))
...
``` {r}
#| title: "TAD Project Types" 

nb_rev <- data.frame(type = c("Small Business", "Affordable Housing", "Retail & Commercial", "Infrastructure"),
                     pct = c(.71, .13, .08, .08))
...
nbrev_plot <- ggplot(nb_rev) +
  ...
  coord_polar("y", start = 0) +
  geom_text(aes(x = "", y = ypos, label = label), size = 5, color = "white") +
  scale_fill_manual(values = c("#0e2b4d", "#009ade", "#7dbf52", "#19d3c5")) +
...
hs_ahu <- data.frame(name = c("2026 YTD", "2026 Goal"), inv = c(484, 1100))

hs_ho <- data.frame(name = c("2026 YTD", "2026 Goal"), support = c(317, 363))
...
units <- data.frame(years = c('2018','2019', '2020', '2021', '2022', '2023', '2024', '2025', '2026'),
                    aff = c(57.3, 55.5, 83.7, 83.0, 94.6, 85.9, 91, 84.1, 91.8))
...
biz_dev <- data.frame(name = c("2026 YTD", "2026 Goal"), closed = c(38, 40))

new_jobs <- data.frame(name = c("2026 YTD", "2026 Goal"), jobs = c(2641, 3000))

good_jobs <- data.frame(name = c("2026 YTD", "2026 Goal"), good = c(1045, 1000))
...
goodjobs_plot <- ggplot(good_jobs) +
  ...
  scale_y_continuous(name = "Number of Good Jobs", limits = c(0,1100), 
                     expand = c(0,0), labels = scales::comma) +
  scale_fill_manual(values = c("#19d3c5", "#7dbf52")) +
  plot_theme()
goodjobs_plot
```
