...
tot_inv <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(856130860.69, 1487100000))

ce_events <- data.frame(name = c("2024 YTD", "2024 Goal"), people = c(4.75, 4.3))
...
#events_plot
```
[![Click the image to explore our interactive online investment map](wins_map_20241108.png){width=55%}](https://iatl.maps.arcgis.com/apps/instant/basic/index.html?appid=112649da7b1d4fa58098c16cc402dcc4){target="_blank"}
...
sb_inv <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(11036950.00, 6000000))

sb_support <- data.frame(name = c("2024 YTD", "2024 Goal"), support = c(1514, 1500))

sb_ta <- data.frame(name = c("2024 YTD", "2024 Goal"), hours = c(2923.64, 2750))

sbinv_plot <- ggplot(sb_inv) +
  geom_col(aes(x = name, y = inv, fill = name), color = "#000000", show.legend = FALSE) +
  scale_x_discrete(name = "Small Business Investment", limits = c("2024 YTD", "2024 Goal")) +
  scale_y_continuous(name = "Direct Investment ($)", limits = c(0,12000000), 
                     expand = c(0,0), labels = scales::comma) +
...
nb_dn <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(71, 50))

nb_tad <- data.frame(name = c("Invest Atlanta", "Total Capital Investment"), 
                     support = c(49901437, 155484565))

nb_res <- data.frame(name = c("2024 YTD", "2024 Goal"), hours = c(9831, 9000))
...
hs_ahu <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(1524, 2600))

hs_ho <- data.frame(name = c("2024 YTD", "2024 Goal"), support = c(328, 175))
...
units <- data.frame(years = c('2018','2019', '2020', '2021', '2022', '2023', '2024'),
                    aff = c(57.3, 55.5, 83.7, 83.0, 94.6, 85.9, 92))
...
biz_dev <- data.frame(name = c("2024 YTD", "2024 Goal"), closed = c(43, 30))

new_jobs <- data.frame(name = c("2024 YTD", "2024 Goal"), jobs = c(3553, 5000))

good_jobs <- data.frame(name = c("2024 YTD", "2024 Goal"), good = c(1664, 1500))
