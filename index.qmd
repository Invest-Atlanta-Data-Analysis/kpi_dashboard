...
sb_inv <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(4106937, 6000000))

sb_support <- data.frame(name = c("2024 YTD", "2024 Goal"), support = c(819, 1500))

sb_ta <- data.frame(name = c("2024 YTD", "2024 Goal"), hours = c(1265.36667, 2750))
...
nb_tad <- data.frame(name = c("Invest Atlanta", "Total Capital Investment"), 
                     support = c(35100000, 81000000))

nb_res <- data.frame(name = c("2024 YTD", "2024 Goal"), hours = c(4775, 9000))
...
nbtad_plot <- ggplot(nb_tad) +
  ...
  scale_y_continuous(name = "Investment ($)", limits = c(0,100000000), 
                     expand = c(0,0), labels = scales::comma) +
  scale_fill_manual(values = c("#5e9732", "#c8df8e")) +
  plot_theme()
nbtad_plot
...
hs_ahu <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(450, 2600))

hs_ho <- data.frame(name = c("2024 YTD", "2024 Goal"), support = c(111, 175))
...
units <- data.frame(years = c('2018','2019', '2020', '2021', '2022', '2023', '2024'),
                    aff = c(57.3, 55.5, 83.7, 83.0, 94.6, 85.9, 99.8))
...
biz_dev <- data.frame(name = c("2024 YTD", "2024 Goal"), closed = c(21, 30))

new_jobs <- data.frame(name = c("2024 YTD", "2024 Goal"), jobs = c(2484, 5000))
...
tot_inv <- data.frame(name = c("2024 YTD", "2024 Goal"), inv = c(327247651.7, 1487100000))

ce_events <- data.frame(name = c("2024 YTD", "2024 Goal"), people = c(4775, 9000))
...
![Click to expand](wins_map_20240711.png){width=85%}
