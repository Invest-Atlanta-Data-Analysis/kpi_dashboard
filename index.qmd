...
tot_inv <- data.frame(name = c("2025 YTD", "2025 Goal"), inv = c(835234141.82, 930000000))
...
#events_plot
```
[![Click the image to explore our interactive online investment map](wins_map_20250910.png){width=55%}](https://iatl.maps.arcgis.com/apps/instant/basic/index.html?appid=112649da7b1d4fa58098c16cc402dcc4){target="_blank"}
...
sb_inv <- data.frame(name = c("2025 YTD", "2025 Goal"), inv = c(7745209, 8500000))

sb_support <- data.frame(name = c("2025 YTD", "2025 Goal"), support = c(506, 1000))
...
nb_tad <- data.frame(name = c("Invest Atlanta", "Total Capital Investment"), 
                     support = c(29903000, 267334679.07))

nb_res <- data.frame(name = c("2025 YTD", "2025 Goal"), ppl = c(14019, 9000))
...
hs_ahu <- data.frame(name = c("2025 YTD", "2025 Goal"), inv = c(770, 1900))

hs_ho <- data.frame(name = c("2025 YTD", "2025 Goal"), support = c(183, 400))
...
units <- data.frame(years = c('2018','2019', '2020', '2021', '2022', '2023', '2024', '2025'),
                    aff = c(57.3, 55.5, 83.7, 83.0, 94.6, 85.9, 91, 74))
...
biz_dev <- data.frame(name = c("2025 YTD", "2025 Goal"), closed = c(37, 30))

new_jobs <- data.frame(name = c("2025 YTD", "2025 Goal"), jobs = c(3033, 3500))
