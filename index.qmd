...
tot_inv <- data.frame(name = c("2025 YTD", "2025 Goal"), inv = c(303993130.82, 930000000))

ce_events <- data.frame(name = c("2025 YTD", "2025 Goal"), people = c(4.53, 4.3))
...
#events_plot
```
[![Click the image to explore our interactive online investment map](wins_map_20250514.png){width=55%}](https://iatl.maps.arcgis.com/apps/instant/basic/index.html?appid=112649da7b1d4fa58098c16cc402dcc4){target="_blank"}
...
sb_inv <- data.frame(name = c("2025 YTD", "2025 Goal"), inv = c(6503491, 8500000))

sb_support <- data.frame(name = c("2025 YTD", "2025 Goal"), support = c(322, 1000))
...
nb_tad <- data.frame(name = c("Invest Atlanta", "Total Capital Investment"), 
                     support = c(10650000, 106784792.07))

nb_res <- data.frame(name = c("2025 YTD", "2025 Goal"), ppl = c(6304, 9000))
...
nbtad_plot <- ggplot(nb_tad) +
  ...
  scale_y_continuous(name = "Investment ($)", limits = c(0,110000000), 
                     expand = c(0,0), labels = scales::comma) +
  scale_fill_manual(values = c("#5e9732", "#c8df8e")) +
  plot_theme()
nbtad_plot
...
hs_ho <- data.frame(name = c("2025 YTD", "2025 Goal"), support = c(85, 400))
...
biz_dev <- data.frame(name = c("2025 YTD", "2025 Goal"), closed = c(13, 30))

new_jobs <- data.frame(name = c("2025 YTD", "2025 Goal"), jobs = c(383, 3500))

good_jobs <- data.frame(name = c("2025 YTD", "2025 Goal"), good = c(179, 1300))
...
sectors <- data.frame(Industry = c("Food Access", "Technology", "Retail", "Other Professional Services"), 
                      total = c(.538, .154, .154, .154))
